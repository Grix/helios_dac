...
int HeliosDac::OpenDevices()
{
	...
	for (int i = 0; i < cnt; i++)
	{
		...
		printf("Found USB device, PID: %d, VID: %d\n", devDesc.idProduct, devDesc.idVendor);

		if ((devDesc.idProduct != HELIOS_PID) || (devDesc.idVendor != HELIOS_VID))
			continue;

		printf("PID and VID matching Helios, opening... ");

		libusb_device_handle* devHandle;
		result = libusb_open(devs[i], &devHandle);
		if (result < 0)
		{
			printf("libusb_open failed with code %d\n", result);
			continue;
		}
		...
		if (result < 0)
		{
			printf("libusb_claim_interface failed with code %d\n", result);
			continue;
		}
		...
		if (result < 0)
		{
			printf("libusb_set_interface_alt_setting failed with code %d\n", result);
			continue;
		}
